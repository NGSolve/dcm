...
namespace ngcomp
{
  ...
  class H1DualCellSegm : public H1CellFiniteElement<1>, public VertexOrientedFE<ET_SEGM>
  {
  ...
  class H1DualCellTrig : public H1CellFiniteElement<2>, public VertexOrientedFE<ET_TRIG>
  {
    const IntegrationRule & GaussRadauIR;
    public:
    H1DualCellTrig (int order, const IntegrationRule & _GaussRadauIR)
      : H1CellFiniteElement<2> (3*sqr(order+1), order), GaussRadauIR(_GaussRadauIR)
    { ; }
  ...
  shared_ptr<BaseMatrix> H1DualCells ::
    GetMassOperator(shared_ptr<CoefficientFunction> rho, shared_ptr<Region> defon, LocalHeap & lh) const
    {
      ...
      switch (dim)
      {
        ...
        default:
          throw Exception("Mass Operator implemented");
      }
    }
